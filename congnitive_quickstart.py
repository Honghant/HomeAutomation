...
def Read(Subscription,Region):
    ...
    model = speechsdk.intent.LanguageUnderstandingModel(app_id="4bd8fe50-b52e-475d-a529-a772b3c6157f")
    intents = [
        (model, "HA_2.AllOFF"),
        (model, "HA_2.AllON"),
        (model, "Color"),
...
# Check the results
    if intent_result.reason == speechsdk.ResultReason.RecognizedIntent:
        ...
        if (intent == "All On"):
            ...
            result = "AllYellow"
        elif (intent == "Change Color" and (entity['location'] == ['on the desk'] or entity['location'] == ['on the left'])):
            result = "DeskGreen"
        elif (intent == "Change Color" and (entity['location'] == ['on the right'] or entity['location'] == ['on the floor'])):
            result = "FloorGreen"
